...
#pragma endregion
	...
	protected:
		database_binder( sqlite3 * db, std::wstring const & sql ):
			_db( db ),
			_sql( sql ),
			_stmt( nullptr ),
			_inx( 1 ) {
...
#pragma region get_col_from_db			
		...
		void get_col_from_db( int inx, std::wstring& w ) {
			if( sqlite3_column_type( _stmt, inx ) == SQLITE_NULL ) w = std::wstring();
			else {
				sqlite3_column_bytes16( _stmt, inx );
				w = std::wstring( (wchar_t *)sqlite3_column_text16( _stmt, inx ) );
			}
		}
...
#pragma endregion
	...
	public:
		...
		database( std::wstring const & db_name ): _db( nullptr ), _connected( false ), _ownes_db( true ) {
		...
		database( std::string const & db_name ): _db( nullptr ), _connected( false ), _ownes_db( true ) {
			_connected = sqlite3_open16( std::wstring( db_name.begin(), db_name.end() ).data(), &_db ) == SQLITE_OK;
		}
		...
		database_binder operator<<(std::wstring const& sql) const {
			return database_binder( _db, sql );
		}
